...
EXPLORE = 20000.0  # frames over which to anneal epsilon
...
class Deep_Q_Network(object):
    ...
    def __init__(self, sess):
        ...
        self.state = tf.placeholder(
            "float", [None, DEPTH_IMAGE_HEIGHT, DEPTH_IMAGE_WIDTH, NumBufferFrames]
        )
        # Conv1 layer
        ...
        self.readout_action = tf.reduce_sum(
            tf.multiply(self.readout, self.actions), axis=1
        )
...
def get_image(client, image_type):
    if image_type == "Scene":
        ...
        try:
            ...
            print(
                "######### Error: I can not get a depth image correctly! #########################"
            )
    ...
    if (
        observation_size[0] == DEPTH_IMAGE_HEIGHT
        and observation_size[1] == DEPTH_IMAGE_WIDTH
    ):
        return observation
    else:
...
def lidar_auxiliary(client, car_controls):
    ...
    if len(lidarData.point_cloud) < 3:
        ...
        if len(zeros_index) <= 1:
            ...
            len(zeros_index) >= len(x_points) - 1
            or y_points[zeros_index[0] - 1] >= y_points[zeros_index[0] + 1]
        ):
            if car_controls.steering >= 0:
                car_controls.steering = -0.5
...
def env_feedback(client, pre_position):
    ...
    distance = np.sqrt(
        (car_position.x_val - terminal_position[0]) ** 2
        + (car_position.y_val - terminal_position[1]) ** 2
    )
    ...
    if collision_info.has_collided:
        ...
        pre_distance = np.sqrt(
            (pre_position[0] - terminal_position[0]) ** 2
            + (pre_position[1] - terminal_position[1]) ** 2
        )
        if distance <= 10:
            reward = 20
...
def store_transition(replay_experiences, store_or_read):
    ...
    if store_or_read == "read":
        if not os.path.exists(store_path) or os.path.getsize(store_path) == 0:
            ...
            print(
                "Successfully load the replay_experiences.pkl, %05d memory" % memory_len
            )
            return replay_experiences
...
def excute_action(client, car_controls, steer):
    ...
    if steer >= straight_range[0] and steer <= straight_range[1]:
        ...
        car_controls.throttle = (
            2 if car_controls.throttle >= 2 else car_controls.throttle
        )
...
def print_action(episode, t, car_controls, distance, steer, reward):
    ...
    print(
        "throttle=%.1f,  steer=%.3f,  reward=%.2f, distance=%.2f"
        % (car_controls.throttle, steer, reward, distance),
        end="",
    )
...
def trainNetwork():
    ...
    state_pre = np.stack(
        (observe_init, observe_init, observe_init, observe_init), axis=2
    )
    ...
    while episode < MAX_EPISODE:
        ...
        while not reset:
            ...
            state_current = np.append(
                observe, state_pre[:, :, : (NumBufferFrames - 1)], axis=2
            )
            current_position, distance, reward_current, terminal, reset = env_feedback(
                client, pre_position
            )
            # store the experience
            if step - 1 > 0:
                replay_experiences.append(
                    (state_pre, action_current, reward_current, state_current, terminal)
                )
            ...
            actions = sess.run(
                online_net.readout, feed_dict={online_net.state: [state_current]}
            )
            readout_t = actions[0]
            ...
            if len(replay_experiences) <= OBSERVE:
                ...
                print(
                    "episode=%05d,step=%05d,we are observing the env,the action is random......"
                    % (episode, step)
                )
            ...
            if episode > OBSERVE:
                ...
                Q1 = online_net.readout.eval(
                    feed_dict={online_net.state: state_current_batch}
                )
                Q2 = target_net.readout.eval(
                    feed_dict={target_net.state: state_current_batch}
                )
                for i in range(0, len(minibatch)):
                    ...
                    if terminal_batch:
                        ...
                        y_batch.append(
                            rewards_batch[i] + GAMMA * Q2[i, np.argmax(Q1[i])]
                        )

                # Update the network with our target values.
                online_net.train_step.run(
                    feed_dict={
                        online_net.y: y_batch,
        ...
        if episode % 5 == 0:
            trainable_saver.save(
                sess,
                "saved_networks/new_model_lidar/Simply_maze",
                global_step=episode,
                write_state=True,
            )
            print(
                "######## The mode has been saved successfully after %d episodes ##########"
                % episode
            )
            #  write summaries
            summary_str = sess.run(
                merged_summary, feed_dict={reward_var: reward_episode}
            )
        ...
        if len(replay_experiences) < 2500:
            ...
            if signal_back:
                print(
                    "######## The replay experiences has been saved successfully after %d episodes ##########"
                    % episode
                )
            else:
                print(
                    "######## Warning: the replay experiences can not be saved after %d episodes ##########"
                    % episode
                )
