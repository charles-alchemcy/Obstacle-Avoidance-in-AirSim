...
def print_action(episode, t, car_controls, distance, steer):
    ...
    print(
        "throttle=%.1f,  steer=%.3f, distance=%.2f"
        % (car_controls.throttle, steer, distance),
        end="",
    )
...
def testNetwork():
    ...
    while episode < MAX_EPISODE:
        ...
        while not reset:
            ...
            terminal, reset, distance, map_grid, latest_distance = env_feedback(
                client, map_grid, latest_distance
            )
            ...
            print(
                ",inner loop=%.4fs\n" % (inner_loop_time_end - inner_loop_time_start),
                end="",
            )
            inner_loop_time_start = time.time()
            time.sleep(0.5)
        ...
        if terminal == 1:
            ...
            collision_record += 1
        print(
            "terminal nums=%d,collision num=%d,average steps is %04d,the shortest distance=%.2f"
            % (
                terminal_record,
                collision_record,
                int(np.mean(steps_record)),
                latest_distance,
            )
        )
        client.reset()
